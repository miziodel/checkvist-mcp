...
7. **Implementation Session Close Protocol**: At the end of every session involving implementation work, the Agent MUST update the project's `roadmaps` and `backlogs` to reflect the completed work. This ensures the strategic view is always synchronized with the codebase state.
